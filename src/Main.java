...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        menu.addButton("Suspect Test", 5, 0, 0, false);
        ...
        if (clic == 3) {
            liste = "battlespot-1760 2015-03.txt";
        }
        if (clic == 4) {
            liste = "anythinggoes-1760 2015-01.txt";
        }
        if (clic == 5) {
            liste = "ou-suspecttest 2015-04.txt";
            threatList = true;
            seuil = 0.89;
        }
        if (clic == 6) {
        ...
        for (int i = 0; i < tiers.size(); ++i) {
            ...
            try {
                ...
                if (test.diversite() > diversite0) {
                    diversite0 = test.diversite();
        ...
        System.out.println("Pokemon le plus versatile : " + pokemonVersatile + " (" + diversite0 + ")");

        int diversite = diversite0;
        ...
        while (adversairesGlobaux.size() > 0 && equipe.size() < 6) {
            ...
            if (threatList) {
                String pokemon = tiers.get(0);
                int set = 0;
                boolean correct = false;
                while (!correct) {
                    System.out.println("\n\nQuel Pokémon étudier ?");
                    String reponse = sc.nextLine();
                    try {
                        pokemon = reponse.split(" ")[0];
                        set = Integer.valueOf(reponse.split(" ")[1]).intValue();
                        Pokemon cobaye = new Pokemon(pokemon, set, liste, "Smogon");
                        cobaye.print();
                        System.out.println("\nEst-ce correct ? (o/n)");
                        correct = sc.nextLine().equals("o");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                }

                System.out.println("\nComment s'appelle ce set ?");
                String nomSet = sc.nextLine();

                int NpokeAdv = 0;

                int score = 0;
                Liste ennemis = new Liste();
                Pokemon P1 = new Pokemon(pokemon, set, liste, "Smogon");

                System.out.println();

                boolean PDR1bis = false;
                boolean picots1bis = false;
                boolean picsToxics1bis = false;
                boolean toileGluante1bis = false;

                boolean PDR2bis = false;
                boolean picots2bis = false;
                boolean picsToxics2bis = false;
                boolean toileGluante2bis = false;
                ...
                int numRatioInit = (int) (maxPoints);
                int denRatioInit = 1;
                ...
                int iPal = 0;
                while (pallier.get(iPal) > 0) {
                    ++iPal;
                }
                System.out.println(" (--> " + (int) (100 / (1 + ratioInit)) + "%)");
                ...
                for (int iDef = 0; iDef < adversairesGlobaux.size(); ++iDef) {
                    ...
                    for (int sDef = 0; sDef < diversite0; ++sDef) {
                        ...
                        int compteSet = 0;
                        while ((!competitif || !probable) && compteSet < 100) {
                            ++compteSet;
                            ...
                            probable = ((new Pokemon(adversairesGlobaux.get(iDef).split(" ")[0], setDef, liste, "Smogon")).isProbable());
                        }

                        P1 = new Pokemon(pokemon, set, liste, "Smogon");
                    ...
                    Float coef = (float) (1000 - preScore);
                    ennemis.add(ajout.getNomSeul() + " " + coef);

                    int numRatio = (int) (score);
                    if (numRatio <= 0) {
                        numRatio = 0;
                    }
                    int denRatio = 1;
                    ...
                    System.out.println("	" + score + " pts" + " (" + ((int) (preScore / 10)) + "% --> " + (coef) + " - " + "AR: " + (tailleTier - NpokeAdv) + " --> " + (score + pallier.get(NpokeAdv - 1)) + ")");

                    if (score + pallier.get(NpokeAdv - 1) <= 0) {
                ...
                try {
                    P1 = new Pokemon(pokemon, set, liste, "Smogon");
                    PrintWriter save = new PrintWriter(new BufferedWriter(new FileWriter("Database/Sauvegardes/ThreatLists/" + pokemon + " " + nomSet + ".txt")));
                    //								if (iAtq >= tiers.size()){
                    //									++sAtq;
                    //								}
                    save.println("ThreatList");
                    save.println(P1.getNom() + " - " + nomSet);
                    save.println(score);
                    save.println("");
                    save.println(P1.toString());
                    save.println("");

                    save.println("Adversaires de " + P1.getNom() + " :");
                    for (int i = 0; i < ennemis.size(); ++i) {
                        if (!ennemis.get(i).contains(" 0.00")) {
                            save.println(ennemis.get(i));
                        }
                    }
            ...
            while (horizontal) {
                int div = setZero;
                for (int iAtqInv = pkmnZero; iAtqInv < tiers.size(); ++iAtqInv) {
                    if (pkmnZero > 0) {
                        pkmnZero = 0;
                    }

                    int iAtq = tiers.size() - 1 - iAtqInv;
                    ...
                    for (int n = 0; n < equipe.size(); ++n) {
                        if (equipe.get(n).split(" ")[0].split("-")[0].equals(tiers.get(iAtq).split(" ")[0].split("-")[0])) {
                            speciesClause = false;
                            System.out.println("\n" + tiers.get(iAtq) + " abandonne pour species clause.");
                        }
                    }
                    
                    if (tiers.get(iAtq).equals(meilleurPokemon.split(" ")[0])){
                        speciesClause = false;
                        horizontal = false;
                        iAtqInv = tiers.size();
                        setZero = -1;
                    ...
                    if (speciesClause) {
                        ...
                        int compteur = 0;
                        int limiteCompteur = (new Pokemon(tiers.get(iAtq), 0, liste, "Smogon")).getNTotalSets();

                        //System.out.println("\n\nProba max = " + (new Pokemon(tiers.get(iAtq), 0, liste, "Smogon")).getUtilisationSet());

                        while (!valide && compteur < limiteCompteur) {
                            ++compteur;

                            setAtq = alea.nextInt((new Pokemon(tiers.get(iAtq), 0, liste, "Smogon")).getNTotalSets());

                            Pokemon PkmnATester = new Pokemon(tiers.get(iAtq), setAtq, liste, "Smogon");

                            boolean competitif = PkmnATester.isCompetitif();
                            boolean probable = PkmnATester.isProbable();
                            boolean megaClause = true;
                            boolean itemClause = true;

                            if (!freeMegaSlot && (PkmnATester.getObjet().endsWith("ite") || PkmnATester.getObjet().endsWith("ite X") || PkmnATester.getObjet().endsWith("ite Y")) && !PkmnATester.getObjet().endsWith("Eviolite")) {
                                megaClause = false;
                            }

                            if (liste.startsWith("battlespot")) {
                                for (int i = 0; i < equipe.size(); ++i) {
                                    String nom = equipe.get(i).split(Pattern.quote(" ["))[0];
                                    if (nom.startsWith("Mega-")) {
                                        nom = nom.split("Mega-")[1];
                                    ...
                                    if (test.getObjet().equals(PkmnATester.getObjet())) {
                                        itemClause = false;
                                    }
                                }
                            }

                            valide = competitif && megaClause && itemClause && probable;
                        }
                        ...
                        if (!valide) {
                            ...
                            for (int iDef = 0; iDef < adversairesGlobaux.size(); ++iDef) {
                                ...
                                for (int sDef = 0; sDef < diversite; ++sDef) {
                                    ...
                                    int compteSet = 0;
                                    limiteCompteur = (new Pokemon(adversairesGlobaux.get(iDef).split(" ")[0], 0, liste, "Smogon")).getNTotalSets();
                                    ...
                                    while (!valide && compteur < limiteCompteur) {
                                        ...
                                        valide = competitif && probable;
                                    }
                                    if (!valide) {
                                        //System.out.print("?")
                                        //System.out.println("\nAucun set valide trouve pour " + tiers.get(iDef) + ".");
                                        //System.out.println("Proba max = " + (new Pokemon(tiers.get(iDef), 0, liste, "Smogon")).getUtilisationSet());
                                    }
                                ...
                                //if (iDef >= iPal && ratio < ratioInit) {
                                if (meilleurScore > 0 && ratio < ratioInit) {
                                    iDef = adversairesGlobaux.size();
                                }
                            }
                            ennemis.triFusion();
                            ...
                            try {
                                ...
                                //									++sAtq;
                                //								}
                                save.println("Horizontal " + div);
                                save.println(iAtqInv + " - " + P1.getNom());
                                save.println(score);
                                save.println("");
                                ...
                                save.print(ennemisRestants.get(0));
                                for (int i = 1; i < ennemisRestants.size(); ++i) {
                                    if (!ennemisRestants.get(i).contains(" 0.00")) {
                                ...
                                save.print(adversairesGlobaux.get(0));
                                for (int i = 1; i < adversairesGlobaux.size(); ++i) {
                                    if (!adversairesGlobaux.get(i).contains(" 0.00")) {
            ...
            if (!horizontal) {
                ...
                for (int sAtqInv = setZero + 1; sAtqInv < Nset; ++sAtqInv) {
                    int sAtq = Nset - 1 - sAtqInv;
                    if (setZero > 0) {
                        setZero = 0;
                    }
                    ...
                    if (cousin.isCompetitif() && megaClause && cousin.distance(veryBest) <= 2 && cousin.distance(veryBest) > 0) {
                        ...
                        for (int iDef = 0; iDef < adversairesGlobaux.size(); ++iDef) {
                            ...
                            for (int sDef = 0; sDef < diversite; ++sDef) {
                                ...
                                int compteur = 0;
                                int limiteCompteur = (new Pokemon(adversairesGlobaux.get(iDef).split(" ")[0], 0, liste, "Smogon")).getNTotalSets();
